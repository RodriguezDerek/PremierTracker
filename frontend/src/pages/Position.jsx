...
function Position() {
  ...
  async function fetchPlayerData() {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/premier-league/players/position/${position}`);
  ...
  function handleInputPositionChange(event) {
    event.preventDefault();
    setInputPosition(event.target.value);
  }

  function handlePositionSubmission() {
    if (inputPosition.trim() !== '') {
      setPosition(inputPosition.trim());
      setInputPosition('');
    }
  }
    ...
    <div className="w-full custom-bg h-full flex flex-col min-h-screen bg-cover bg-center">
      ...
      <div className="absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 w-full px-4 fadeInUp">
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center mt-4 gap-4">
          <input value={inputPosition} onChange={handleInputPositionChange} type="text" placeholder="Enter a Position..." className="w-full sm:w-80 h-12 sm:h-14 px-4 py-2 border bg-gray-200 border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"/>
